...
List<ClassStudent> classes = (List<ClassStudent>) request.getAttribute("classesStudent");
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadAllEvents() {
        fetch("/admin/student/loadAllEvents")
            .then(response => response.text())
            .then(data => eventTableBody.innerHTML = data)
            .catch(error => console.error("Error loading all events:", error));
    }

    function searchEvents(keyword) {
        const url = "/admin/student/searchEvent?keyword=" + encodeURIComponent(keyword);
        fetch(url)
            .then(response => response.text())
            .then(data => {
...
<div class="manage-container">
	...
	<table>
		...
		<tbody id="eventTableBody">
			<%
				int i = 1;
				for (ClassStudent stuClass : classes) {
			%>
			<tr>
				<td><%= i++ %></td>
				<td><%= stuClass.getClassName() %></td>
				<td><%= stuClass.getClassDayofweek() %></td>
				<td><%= stuClass.getClassStarttime() %></td>
				<td><%= stuClass.getClassEndtime() %></td>
				<td><%= stuClass.getClassLocation() %></td>
				<td><%= stuClass.getClassDesc() %></td>
